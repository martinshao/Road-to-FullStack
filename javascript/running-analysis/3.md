...
JS引擎执行JavaScript代码的主线程会被分配一个stack，这就是**同步执行栈**，也叫做调用栈（call stack），同时JS引擎还会维护一个异步队列，用于维护异步事件，这就是**异步队列**（event queue）。在JavaScript存在一些Web APIs。

于是，JavaScript所有任务可以分成两种，一种是**同步任务（synchronous）**，另一种是**异步任务（asynchronous）**。
