...
解释上文提到的一个问题：JavaScript这一门单线程的语言（一段时间内只能做一件事）。
...
在js中其实有两类**任务队列（task queue）**：宏任务队列（macro tasks）和微任务队列（micro tasks）。不同的异步任务会被安排在不同的异步任务队列中。
...
上文中提到JavaScript是单线程，并且有同步任务和异步任务的区分，还有同步执行栈和异步任务队列的概念，那么如何组织这些元素使得JavaScript正常运行呢？这时候Event loop机制登场了。Event loop在browser端和node端也有区分。浏览器的 Event Loop 遵循的是 HTML5 标准，而 NodeJs 的 Event Loop 遵循的是 libuv。
