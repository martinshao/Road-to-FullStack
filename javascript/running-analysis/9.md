...
关于执行上下文和调用栈之间的关系，我们在《JavaScript执行机制深度解析——执行上下文与调用栈》一文中已经做出比较详情的解释，这里我再稍微介绍一下：

ECMAScript代码由三种类型: 全局(global) 代码, 函数(function) 代码和 求值(eval) 代码，除去eval代码我们不讨论，其他两种代码其实就在执行过程中构成了全局执行上下文、函数执行上下文。
