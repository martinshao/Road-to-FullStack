...
从Call Stack到EC，然后我们构建EC的研究条件，分别按照EC的ES3和ES5标准讨论

本文基于EC的ES3实现标准进行讨论，详细的探讨EC和EC中出现的各种关键名词深入讨论。
...
![alt text](../_assets/20190525180349.png "JavaScript call stack ")

除了这3个所需要的属性(变量对象(variable object)， this指针(this value)， 作用域链(scope chain) )，执行上下文根据具体实现还可以具有任意额外属性。

## 变量和执行上下文

在引出变量对象概念之前，我们先做一个铺垫。编程的世界核心元素是什么？目前的我认为是数据结构和算法，当然这已经是一种相当抽象的说法，在JavaScript编程中，切实的说就是变量和函数。但是我们在日常编程过程中写的代码是没有任何意义的，只有在运行的时候，问题才能得到解决。那么运行的时候，变量和函数又何去何从？从之前的文章中我们已经介绍过执行栈和执行上下文，具体的是执行上下文就是函数运行时的环境了，这个环境如何具象化，由此引出变量对象的概念，把它看做一个对象，这个对象存储着JavaScript函数运行的一切条件。
...
在java中我们常把复杂业务中处理不同事物，抽象成接口，变量对象有异曲同工之妙。
